 ...
 * located within a {@link Text} field. This {@link TextWidgetMatcherEditor} is
 * directly coupled with a {@link Text} and fires {@link MatcherEditor} events
 * in response to {@link ModifyEvent}s received from the {@link Text}. This
 ...
 */
public final class MyTextWidgetMatcherEditor extends MyTextMatcherEditor {
    ...
    private boolean forceRefilter;

    /**
     * Creates a TextWidgetMatcherEditor bound to the provided {@link Text} with
     * the given <code>textFilterator</code>.
     * 
     * @param text
     *            the {@link Text} widget that drives the text-filtering
     ...
     */
    public MyTextWidgetMatcherEditor(Text text, TextFilterator textFilterator) {
     ...
     *            implement {@link ca.odell.glazedlists.TextFilterable}.
     * @param live
     *            <code>true</code> to filter by the keystroke or
     *            <code>false</code> to filter only when the ENTER key is
     *            pressed within the Text. Optionnally, you can use the
     *            {@link SelectionListener} provided via
     *            {@link #getFilterSelectionListener()} to register a Button or
     *            other component to trigger filtering.
     * 
     * @see GlazedLists#textFilterator(String[])
     */
    public MyTextWidgetMatcherEditor(Text text, TextFilterator textFilterator,
     ...
     * @param live
     *            <code>true</code> to filter by the keystroke or
     *            <code>false</code> to filter only when
     *            {@link java.awt.event.KeyEvent#VK_ENTER Enter} is pressed
     *            within the {@link JTextField}. Note that non-live filtering
     *            is only supported if <code>textComponent</code> is a
     *            {@link JTextField}.
     */
     ...
     * on the {@link Text} component, thus freeing the MatcherEditor to be
    ...
    private void refilter() {
        setFilterText(text.getText().split("[ \t]"), forceRefilter);
    }
    ...
    public void setForceRefilter(boolean force) {
        forceRefilter = force;
    }
}